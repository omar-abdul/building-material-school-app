...
function setupEventListeners() {
  ...
  document
    ...
    ?.addEventListener("input", debounce(filterCustomerBalances, 300));
  document
    .getElementById("supplier-search")
    ?.addEventListener("input", debounce(filterSupplierBalances, 300));
  document
...
async function loadFinancialOverview() {
  try {
    const periodParams = getPeriodFilterParams();
    const response = await fetch(
      buildApiUrl(
        `financial/financial.php?action=getFinancialOverview&${periodParams}`
      )
    );
...
function updateOverviewCards(data) {
  ...
  document.getElementById("total-cogs").textContent = formatCurrency(
    data.total_cogs || 0
  );
  document.getElementById("total-expenses").textContent = formatCurrency(
    data.total_expenses || 0
  );
  document.getElementById("net-profit").textContent = formatCurrency(
    data.net_profit || 0
  );
  document.getElementById("pending-payments").textContent = formatCurrency(
...
async function showCustomerBalances() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="6" class="loading">Loading customer balances...</td></tr>';

  try {
    const response = await fetch(
      buildApiUrl("financial/customer-balances.php")
    );
    const data = await response.json();
    ...
    if (data.success) {
      ...
      tbody.innerHTML =
        '<tr><td colspan="6" class="empty-state">No customer balances found</td></tr>';
    }
  } catch (error) {
    console.error("Error loading customer balances:", error);
    tbody.innerHTML =
      '<tr><td colspan="6" class="empty-state">Error loading customer balances</td></tr>';
  }
}
...
async function showSupplierBalances() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="6" class="loading">Loading supplier balances...</td></tr>';

  try {
    const response = await fetch(
      buildApiUrl("financial/supplier-balances.php")
    );
    const data = await response.json();
    ...
    if (data.success) {
      ...
      tbody.innerHTML =
        '<tr><td colspan="6" class="empty-state">No supplier balances found</td></tr>';
    }
  } catch (error) {
    console.error("Error loading supplier balances:", error);
    tbody.innerHTML =
      '<tr><td colspan="6" class="empty-state">Error loading supplier balances</td></tr>';
  }
}
...
async function showTransactionHistory() {
  ...
  try {
    ...
    const response = await fetch(
      buildApiUrl(
        `financial/financial.php?action=getTransactions&${periodParams}`
      )
    );
    const data = await response.json();
    ...
    .map(
      (customer) => `
        <tr>
            ...
            <td>
                <span class="balance-amount ${getBalanceClass(
                  customer.balance
                )}">
    ...
    .map(
      (transaction) => `
        <tr>
            ...
            <td>
                <span class="balance-amount ${getAmountClass(
                  transaction.Amount
                )}">
            ...
            <td>
                <div class="action-buttons">
                    <button class="btn btn-sm btn-primary" data-transaction-id="${
...
async function generateReport() {
  const periodParams = getPeriodFilterParams();
  console.log("Generating report with period params:", periodParams);
  ...
  try {
    const response = await fetch(
      buildApiUrl(`financial/report.php?${periodParams}`)
    );
...
function renderFinancialReport(data) {
  ...
  document.getElementById("profit-analysis").innerHTML = `
        <div class="summary-item">
            ...
            <span class="value ${
              data.profit.net >= 0 ? "positive" : "negative"
            }">${formatCurrency(data.profit.net)}</span>
        </div>
        <div class="summary-item">
        ...
        <div class="summary-item">
            ...
            <span class="value ${
              data.profit.growth >= 0 ? "positive" : "negative"
            }">${data.profit.growth}%</span>
        </div>
    `;
...
async function viewTransactionDetails(transactionId) {
  ...
  try {
    const response = await fetch(
      buildApiUrl(`financial/transaction-details.php?id=${transactionId}`)
    );
    const data = await response.json();
  ...
  details.innerHTML = `
        <div class="transaction-detail">
            ...
            <div class="detail-item">
                ...
                <div class="detail-value">${escapeHtml(
                  transaction.customer_supplier_name || "N/A"
                )}</div>
            </div>
            <div class="detail-item">
...
async function viewCustomerTransactions(customerId) {
  try {
    const response = await fetch(
      `../api/financial/financial.php?action=getCustomerTransactions&customerId=${customerId}`
    );
    const data = await response.json();
...
async function viewSupplierTransactions(supplierId) {
  try {
    const response = await fetch(
      `../api/financial/financial.php?action=getSupplierTransactions&supplierId=${supplierId}`
    );
    const data = await response.json();
...
async function loadCustomersForPayment() {
  try {
    const response = await fetch(
      "../api/customers/customers.php?action=getCustomers"
    );
    const data = await response.json();
    ...
    if (data.success) {
      ...
      for (const customer of data.data) {
        ...
        option.textContent = `${customer.CustomerName || customer.Name} (${
          customer.Email
        })`;
...
async function loadSuppliersForPayment() {
  try {
    const response = await fetch(
      "../api/suppliers/suppliers.php?action=getSuppliers"
    );
    const data = await response.json();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("customer-payment-form")
  ...
  document
    .getElementById("supplier-payment-form")
    .addEventListener("submit", async (e) => {
      e.preventDefault();
...
async function recordCustomerPayment() {
  const customerIdValue = document.getElementById(
  ...
  const formData = {
    ...
    description:
      document.getElementById("customer-payment-description").value ||
      "Customer Payment",
  ...
  try {
    const response = await fetch(
      "../api/financial/financial.php?action=addTransaction",
      {
        method: "POST",
        headers: {
...
async function recordSupplierPayment() {
  const supplierIdValue = document.getElementById(
  ...
  const formData = {
    ...
    description:
      document.getElementById("supplier-payment-description").value ||
      "Supplier Payment",
