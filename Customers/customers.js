	...
	fetch(buildApiUrl(`customers/customers.php?id=${id}`))
		...
		.then((data) => {
			if (data.success) {
				...
				// Get order history
				return fetch(
					buildApiUrl(`customers/customers.php?id=${id}&history=true`),
				);
			}
			...
			if (data.success) {
				...
				if (data.data.length === 0) {
					...
					for (const order of data.data) {
						...
						totalPaid += Number.parseFloat(order.PaidAmount || 0);
						outstandingBalance += Number.parseFloat(order.Balance || 0);
						...
						row.innerHTML = `
                            ...
                            <td>${order.PaidFormatted}</td>
                            <td>${order.BalanceFormatted}</td>
...
function filterCustomers() {
	...
	fetch(
		`buildApiUrl('customers/customers.php?term=${encodeURIComponent(searchTerm)}`,
	)
		.then((response) => response.json())
		.then((data) => {
			if (data.success) {
