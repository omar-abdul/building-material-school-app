...
function setupActionButtonListeners() {
	...
	if (tableBody) {
		tableBody.addEventListener("click", (e) => {
			...
			console.log("Closest action button:", target);
			if (!target) return;

			const employeeId = target.dataset.employeeId;
			console.log("Employee ID from dataset:", employeeId);
			if (!employeeId) return;

			if (target.classList.contains("view-btn")) {
				console.log("View button clicked, calling viewEmployee");
				viewEmployee(employeeId);
			} else if (target.classList.contains("edit-btn")) {
				console.log("Edit button clicked, calling editEmployee");
				editEmployee(employeeId);
			} else if (target.classList.contains("delete-btn")) {
				console.log("Delete button clicked, calling deleteEmployee");
				deleteEmployee(employeeId);
			}
		});
		console.log("Event listener added to table body");
	} else {
				...
				for (const employee of data.data) {
					...
					row.innerHTML = `
                        ...
                        <td class="action-cell">
                            <button class="action-btn view-btn" data-employee-id="${employee.id}">
                                <i class="fas fa-eye"></i> View
                            </button>
                            <button class="action-btn edit-btn" data-employee-id="${employee.id}">
                                <i class="fas fa-edit"></i> Edit
                            </button>
                            <button class="action-btn delete-btn" data-employee-id="${employee.id}">
		...
		.then((data) => {
			if (data.success) {
				...
				const percentageIncrease =
					((expectedSalary - baseSalary) / baseSalary) * 100;
				...
				document.getElementById("DateAdded").value = employee.dateAdded
					? employee.dateAdded.split(" ")[0]
...
function viewEmployee(id) {
	console.log("viewEmployee called with ID:", id);
	fetch(buildApiUrl(`employees/employees.php?id=${id}`))
		.then((response) => response.json())
		.then((data) => {
			...
			if (data.success) {
				...
				document.getElementById("viewBaseSalary").textContent =
					`$${Number.parseFloat(employee.baseSalary).toFixed(2)}`;
				document.getElementById("viewExpectedSalary").textContent =
					`$${Number.parseFloat(employee.expectedSalary).toFixed(2)}`;
				document.getElementById("viewPhone").textContent = employee.phone;
				document.getElementById("viewEmail").textContent = employee.email;
				document.getElementById("viewGuarantor").textContent =
					employee.guarantor;
...
function deleteEmployee(id) {
	...
	for (const row of rows) {
		...
		if (actionCell) {
			...
			if (viewBtn && viewBtn.dataset.employeeId === id) {
...
function saveEmployee(e) {
	...
	const baseSalary = Number.parseFloat(
		document.getElementById("baseSalary").value,
	);
	const salaryIncrease = Number.parseFloat(
		document.getElementById("salaryIncrease").value,
	);
	const expectedSalary = Number.parseFloat(
		document.getElementById("expectedSalary").value,
	);
