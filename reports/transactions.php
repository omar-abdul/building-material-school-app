...
<body>
  <div class="container">
    ...
    <main>
      <div class="header">
        <h1 id="main-title">BMMS - Transactions Report</h1>
      </div>
      ...
      <div class="report-box">
        ...
        $where = "WHERE 1=1";
        if (!empty($startDate)) {
          $where .= " AND T.TransactionDate >= '$startDate 00:00:00'";
        }
        if (!empty($endDate)) {
          $where .= " AND T.TransactionDate <= '$endDate 23:59:59'";
        }

        $sql = "
          SELECT T.TransactionID, O.OrderID, C.CustomerName, T.Amount, T.PaymentMethod, T.TransactionDate, T.Status
          FROM Transactions T
        ...
        <table id="transactionsTable" class="display nowrap inventory-table">
          <thead>
            <tr>
              <th>ID</th>
              <th>Order</th>
