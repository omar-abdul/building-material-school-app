...
<body>
  <div class="container">
    ...
    <main>
      <div class="header">
        <h1 id="main-title">BMMS - Inventory Report</h1>
      </div>
      ...
      <div class="report-box">
        ...
        $where = "WHERE 1=1";
        if (!empty($startDate)) {
          $where .= " AND Inventory.LastUpdated >= '$startDate 00:00:00'";
        }
        if (!empty($endDate)) {
          $where .= " AND Inventory.LastUpdated <= '$endDate 23:59:59'";
        }
        if (!empty($searchName)) {
          $where .= " AND Items.Item_name LIKE '%$searchName%'";
        }

        $query = "
          SELECT Inventory.InventoryID, Items.ItemID, Items.ItemName,Categories.CategoryName,Employees.EmployeeName,Inventory.Quantity, Inventory.LastUpdated 
          FROM Inventory 
          JOIN Items ON Inventory.ItemID = Items.ItemID 
          JOIN Categories ON Items.CategoryID = Categories.CategoryID
          JOIN Employees ON Items.RegisteredByEmployeeID = Employees.EmployeeID
          $where
        ...
        <form method="get" class="filter-form">
          ...
          <label>Search Item:
            <input type="text" name="search_name" placeholder="Item name..." value="<?= htmlspecialchars($searchName) ?>">
          </label>
          <button type="submit" class="btn-filter">🔍 Filter</button>
        </form>

        <table id="inventoryTable" class="display nowrap inventory-table">
          <thead>
            <tr>
              <th>Inventory ID</th>
              <th>Item ID</th>
