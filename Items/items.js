...
categoryIdInput.addEventListener("input", () =>
	searchCategories(categoryIdInput.value),
);
employeeIdInput.addEventListener("input", () =>
	searchEmployees(employeeIdInput.value),
);
	...
	for (const item of items) {
		...
		row.innerHTML = `
            ...
            <td class="action-cell">
                <button class="action-btn view-btn" data-item-id="${item.ItemID}">
                    <i class="fas fa-eye"></i> View
                </button>
                <button class="action-btn edit-btn" data-item-id="${item.ItemID}">
                    <i class="fas fa-edit"></i> Edit
                </button>
                <button class="action-btn delete-btn" data-item-id="${item.ItemID}">
...
function openAddItemModal() {
	...
	try {
		...
		document.getElementById("createdDate").value = new Date()
			.toISOString()
			.split("T")[0];
		...
		.then((data) => {
			if (data.success) {
				...
				document.getElementById("viewQuantity").textContent =
					item.Quantity || 0;
				document.getElementById("viewSupplier").textContent = item.SupplierName
					? `${item.SupplierName} (SUP-${item.SupplierID})`
					: "N/A";
				document.getElementById("viewEmployee").textContent = item.EmployeeName
					? `${item.EmployeeName} (EMP-${item.RegisteredByEmployeeID})`
					: "N/A";
...
async function searchCategories(query) {
	...
	try {
		const response = await fetch(
			buildApiUrl(
				`categories/categories.php?search=${encodeURIComponent(query)}`,
			),
		);
...
async function searchEmployees(query) {
	...
	try {
		const response = await fetch(
			buildApiUrl(
				`employees/employees.php?search=${encodeURIComponent(query)}`,
			),
		);
		const data = await response.json();
